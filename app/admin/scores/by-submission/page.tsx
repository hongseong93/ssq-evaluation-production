...
export default function ScoresBySubmissionPage() {
  ...
  const rows = useMemo(() => (data?.submissions ?? []).map((submission: any) => {
    const evaluations = (data?.evaluations ?? []).filter((item: any) => item.submission_id === submission.id && item.status === "submitted");
    ...
    return { submission, totals, average };
  ...
  return (
    <AdminShell title="출품작별 총점표" eyebrow="Scores">
      ...
      <Card className="overflow-hidden">
        <DataTable
          headers={["순위", "접수번호", "부문", "작가명", "작품명", "평균", "최고점", "최저점", "심사위원 수"]}
